    ...
    protected void renderLivingLabel(T entityIn, String str, double x, double y, double z, int maxDistance)
    {
        if(!Asyncware.instance.moduleManager.getModuleByName("Nametags").isToggled()) {
            ...
            if (d0 <= (double) (maxDistance * maxDistance)) {
                ...
                worldrenderer.pos((double) (-j - 2), (double) (8 + i), 0.0D).color(0.0F, 0.0F, 0.0F, 0.25F).endVertex();
                worldrenderer.pos((double) (j + 2), (double) (8 + i), 0.0D).color(0.0F, 0.0F, 0.0F, 0.25F).endVertex();
                worldrenderer.pos((double) (j + 2), (double) (-1 + i), 0.0D).color(0.0F, 0.0F, 0.0F, 0.25F).endVertex();
                tessellator.draw();
                GlStateManager.enableTexture2D();

                //   Gui.drawRect(50, 10, -50, -20, new Color(24, 24, 24, 50).getRGB());
                //   Asyncware.rendererTags.drawString(str, -Asyncware.renderer.getStringWidth(str) * 2 + 30 , i - 20, -1, false);
                //Asyncware.renderer.drawString(str, -Asyncware.renderer.getStringWidth(str) / 2, i - 2, -1, false);
                fontrenderer.drawString(str, -fontrenderer.getStringWidth(str) / 2, i, 553648127);
                GlStateManager.enableDepth();
                GlStateManager.depthMask(true);
                // Asyncware.renderer.drawString("Winning", -Asyncware.renderer.getStringWidth(str) / 2, i - 2, -1, false);
